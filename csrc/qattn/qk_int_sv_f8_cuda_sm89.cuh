...
template<uint32_t CTA_Q, uint32_t CTA_K, uint32_t WARP_Q, uint32_t WARP_K, uint32_t head_dim, DataType DTypeQK, QuantGranularity Q_GRAN, QuantGranularity K_GRAN,
        typename DTypeSVAccum = float, bool use_inst_buffer = false, typename DTypeOut = half, ComputeUnit DenominatorAccumUnit, MaskMode mask_mode = MaskMode::kNone, bool return_lse = false, bool fuse_v_scale=false, bool fuse_v_mean=false, bool use_pv_fp16_accu=false, bool IS_SPARSE = false>
                      ...
                      float sm_scale, int32_t* block_index = nullptr, uint32_t top_k = 0)
{
  ...
  #pragma unroll
    ...
    float dequant_scale = q_scale * K_scale[k_scale_idx + current_k_block * k_scale_advance_offset];
    ...
    // update V_lane_ptr to current_k_block
    int8_t * V_lane_ptr = V_lane_base_ptr + current_k_block * CTA_K;
    ...
    uint32_t num_main_iter;
    if constexpr (IS_SPARSE) {
      num_main_iter = num_iterations; // sparse attention: no second last iter since we don't support causal mask for sparse attention
    } else {
      num_main_iter = num_iterations -1;
    }
  
    #pragma unroll
    for (uint32_t iter = 1; iter < num_main_iter; iter++)
    {
      if (next_k_block == -1) {
        break;
      }
  ...
  #pragma unroll
      ...
      // for fp16: 
      // load_global_to_share                stride_seq_v
      // update V_lane_ptr to next_k_block
      ...
      if constexpr (IS_SPARSE) {
        if (iter + 1 < top_k) {
          next_k_block = static_cast<uint32_t>(block_index[block_index_base_offset + iter + 1]);
        } else {
